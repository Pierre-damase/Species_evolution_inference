...
def likelihood_ratio_test(tau, kappa, msprime_model, dadi_model, control_model, optimization,
    ...
    # To keep track of some sfs generated by dadi
    list_sfs = []
    ...
    for i in range(nb_simu):
        ...
        # Save the SFS of the best inference, i.e. the inference with the highest log likelihood
        index = tmp["LL"].index(max(tmp["LL"]))  # Get index of the best inference (higher ll)
        list_sfs.append(tmp["SFS"][index])
