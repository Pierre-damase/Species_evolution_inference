...
def compute_stairway_inference(simulation, file_data, path_stairway, path_data):
    """
    Parameter
    ---------
    file_data: file to summarize the data from inference with stairway plot
    ...
    for i, sfs in enumerate(simulation['SFS observed']):  # Iterate through each observed SFS
        blueprint = "stairway_inference-{}".format(i)
        ...
        if i == 0 and (tau in tau_list and kappa in kappa_list):
            figure = "{0}{1}/{1}.final.summary.png".format(path_data, blueprint)
            os.system("mv {} ./Figures/Stairway/stairway-tau={}_kappa={}".format(figure, tau,
                                                                                 kappa))

        # Remove all blueprint file & stairway file
        os.system("rm -rf {}{}.blueprint*".format(path_data, blueprint))
        os.system("rm -rf {}{}".format(path_data, blueprint))
...
def save_stairway_inference(simulation, model):
    ...
    data = compute_stairway_inference(simulation, file_data, path_stairway, path_data)
